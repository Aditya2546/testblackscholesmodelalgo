...
if st.session_state.prev_data_source != data_source:
    st.sidebar.warning(f"⚠️ Data source changed from {st.session_state.prev_data_source} to {data_source}. You must reset learning data.")
    ...
    if st.sidebar.button("Reset Learning Data", key="reset_button", type="primary"):
        try:
            ...
            st.sidebar.success(f"✅ Learning data reset successfully for {data_source}. The system will learn new patterns.")
        except Exception as e:
            st.sidebar.error(f"❌ Error resetting learning data: {str(e)}")
else:
...
st.sidebar.write(f"📊 **Active Data Source:** {data_source}")
    ...
    if 'alpaca_integration' not in st.session_state or \
       ...
       st.session_state.alpaca_base_url != alpaca_base_url:
        ...
        try:
            ...
            st.sidebar.error(f"❌ Error connecting to Alpaca: {str(e)}")
...
with tab1:
        ...
        if st.session_state.signals:
            for i, signal in enumerate(st.session_state.signals):
                with st.expander(f"{signal.symbol} {signal.option_type.upper()} ${signal.strike} - Score: {signal.signal_strength:.2f}"):
                    ...
                    # Display volume and open interest with safe attribute checking
                    volume = getattr(signal, 'volume', 'N/A')
                    open_interest = getattr(signal, 'open_interest', 'N/A')
    ...
    with col2:
        ...
        st.dataframe(market_data_df.style.map(color_rsi, subset=["RSI"]), height=400)
        ...
        with col1:
            ...
            if any("total_slippage" in trade for trade in st.session_state.trade_history):
                ...
                if filled_trades:
                    ...
                    st.write(f"📊 **Impact Analysis:** Trading costs reduce profit by {cost_impact_pct:.1f}% ({gross_profit-net_profit:.2f})")
                    ...
                    if limit_trades and market_trades:
                        ...
                        st.write(f"📈 **Order Type Analysis:** Limit orders avg profit: ${limit_profit:.2f} | Market orders avg profit: ${market_profit:.2f}")
    ...
    if trading_system != "Smart Trader (ML)":
        ...
        with data_source_container:
            ...
            if os.environ.get("TRADING_DATA_SOURCE") != data_source:
                st.error(f"⚠️ Warning: Learning system is using patterns from a different data source than currently selected. Please reset learning data.")
                if st.button("Reset Learning Data Now", type="primary"):
                    try:
                        ...
                        st.session_state.smart_trader = SmartTrader(account_value=st.session_state.account_value)
                        st.session_state.realistic_smart_trader = RealisticSmartTrader(account_value=st.session_state.account_value)
                        st.success(f"✅ Learning data reset successfully for {data_source}.")
                        st.experimental_rerun()  # Refresh the page
                    except Exception as e:
                        st.error(f"❌ Error resetting learning data: {str(e)}")
            else:
                st.success(f"✅ Learning system is correctly using {data_source} data.")
